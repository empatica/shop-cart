...
namespace ShopCart {
  ...
  export class CartController {
    ...
    dispatcher: ShopCart.Dispatcher;
    ...
    */
    openCart() {
      ...
      this.elementLoop(this.config.selector.cart.main, function(
        e: Element,
        self: ShopCart.CartController
      ) {
    ...
    */
    private registerCartItemEvents(n: Element) {
      ...
      for (var i = 0; i < l.length; i++) {
        ...
        this.events[
          getElemData(l[i], ShopCartConst.DATA_UPC) +
            "-" +
            getElemData(l[i], ShopCartConst.DATA_ACTION)
        ] = true;
      }
    ...
    private registerCartUpdateEvent(el: Element) {
      ...
      el.addEventListener("input", function(e) {
        ...
        if (v) {
          ...
          self.setItemQuantity(
            getElemData(e.target, ShopCartConst.DATA_UPC),
            q
          );
        }
      });
    ...
    */
    private registerCartActionEvent(el: Element) {
      ...
      this.events[
        getElemData(el, ShopCartConst.DATA_UPC) +
          "-" +
          getElemData(el, ShopCartConst.DATA_ACTION)
      ] = true;
    ...
    */
    private updateItemQuantity(i: ShopCart.CartItem) {
      var n = document.querySelectorAll(
          "#" + ShopCartConst.PREFIX_ITEM + i.item.upc
        ),
    ...
    */
    private updateCartActive() {
      this.elementLoop(this.config.selector.element.main, function(
        e: Element,
        self: ShopCart.CartController
      ) {
        if (self.cart.quantity > 0) {
          if (
            e.className.indexOf(self.config.selectorClass.element.active) < 0
          ) {
    ...
    */
    private updateCartNotification() {
      this.elementLoop(this.config.selector.element.notification, function(
        e: Element,
        self: ShopCart.CartController
      ) {
        if (self.cart.quantity > 0) {
    ...
    private setConfig(config: any) {
      ...
      for (var i = 0; i < config.products.length; i++) {
        ...
        for (var j = 0; j < p.variants.length; j++) {
          ShopCart.variantProductMap[p.variants[j].upc] = new ShopCart.Product(
            p.id,
            p.name,
            p.price,
            p.description,
          ...
          ShopCart.productVariantsImageMap[
            p.variants[j].upc
          ] = new ShopCart.ProductImage(p.variants[j].image.url);
          ShopCart.variantMap[p.variants[j].upc] = new ProductVariant(
            p.variants[j].name,
            p.variants[j].upc,
            p.variants[j].sku,
            p.variants[j].size,
            p.variants[j].image.url
          );
        }
      }
    }
