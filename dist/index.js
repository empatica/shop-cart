...
(function (ShopCart) {
    ...
    var Cart = (function () {
        ...
        Cart.prototype.getId = function () {
            ...
            var id = s4() + s4() + '-' + s4() + '-' + s4() + '-' + s4() + '-' + s4() + s4() + s4() + '-' + Date.now();
            return id;
        };
        return Cart;
    }());
    ShopCart.Cart = Cart;
    var cartProps = ['id', 'createdAt', 'total', , 'quantity', 'items', 'currency'];
    function isValidCart(cart) {
        if (!cart || !cart.createdAt || cart.createdAt < ShopCart.CART_INVALID_BEFORE) {
            return false;
        }
        var v = true;
...
(function (ShopCart) {
    var Config = (function () {
        ...
        Config.prototype.getDefaultCurrency = function () {
            ...
            var list = this.currencies.filter(function (c) { return c.default; });
...
(function (ShopCart) {
    ...
    var CartController = (function () {
        ...
        CartController.prototype.updateItemQuantity = function (i) {
            var n = document.querySelectorAll('#' + ShopCartConst.PREFIX_ITEM + i.item.upc), q = document.querySelectorAll('#' + ShopCartConst.PREFIX_ITEM + i.item.upc + ' ' + ShopCartConst.SELECTOR_ITEM_QUANTITY);
            if (q && q.length > 0) {
                if (i.quantity <= 0) {
